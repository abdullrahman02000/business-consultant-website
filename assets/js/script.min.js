...
function tabContainerInit(tabContainer, tabInfo) {
    let tabs = tabContainer.getElementsByTagName('input');
    let currentTab = document.getElementById(tabs[0].getAttribute('data-target'));
    for (let a=0; a<tabs.length; a++)
    {
        let tab = document.getElementById(tabs[a].getAttribute('id'));
        let targetEl = document.getElementById(tab.getAttribute('data-target'));
        if (tab.checked) {
...
window.onload = function() {
    tabContainerInit(document.getElementById('contact-notes'));
    $("#country_selector").countrySelect({
        defaultCountry: "iq"
    });
    let customServiceType = document.getElementById('custom-servicetype');
    let servicetype = document.getElementById('service-type');
    let customservice = document.querySelector('input[name="customservice"]');
    servicetype.addEventListener('change', function() {
        let val = servicetype.selectedIndex;
        if (val==servicetype.childElementCount-1) {
            customServiceType.classList.remove('d-none');
        }else {
            customServiceType.classList.add('d-none');
            customservice.value = "";
        }
    });
